...
Two modes, decided by whether ``$AGY_PROJECT_DIR/.gemini/antigravity-cli/sync.toml`` exists:
...
* **Sync mode** (file present): run ``uv sync --frozen`` with the declared
  ...
  so Antigravity does not need to guess whether pytest/ruff/etc. are available.
* **Detect-only mode** (file absent): parse ``pyproject.toml``; if it declares
  ...
  so Antigravity does not silently assume they are installed in ``.venv/``.
...
from _agy_lib import env, hooks, proc
...
# than being SIGKILLed by Antigravity CLI with no output.
UV_TIMEOUT_SEC: Final[int] = 10


def _project_dir(data: Mapping[str, object]) -> Path | None:
    root = os.environ.get("AGY_PROJECT_DIR")
    if root:
        return Path(root)
...
def _load_sync_config(project: Path) -> dict[str, object] | None:
    sync_toml = project / ".gemini/antigravity-cli" / "sync.toml"
...
def _sync_mode(project: Path, config: Mapping[str, object]) -> str:
    ...
    return (
        ...
        "Venv is in sync with .gemini/antigravity-cli/sync.toml — `uv run <cmd>` will resolve "
        "declared tools."
    )
