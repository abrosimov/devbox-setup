...
| `ai_config/manifest.py:34–149`, `core.py:76–164` | Schema 1, longest-prefix field ownership, environment bindings, template provenance and ordered-set strategy | Existing manifest is a good ownership catalogue but not a client value schema; broad object prefixes admit unknown descendant names |
| `ai_config/bindings.py` | Resolves keychain/home values into repository projection | The profile and env providers were removed on 21 September 2026: per-profile values are Jinja in the source, rendered by `ai_config/templating.py` from the role defaults plus `profiles/<profile>.yml`. Injection-based providers already support isolated tests |
