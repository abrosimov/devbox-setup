...
The current `warp+doh` mode sends IP traffic through WARP and lets WARP handle DNS over HTTPS. Pub selects WARP's WireGuard tunnel protocol because the affected wifi carries complete Anthropic request bodies through WireGuard while its MASQUE path resets larger uploads. The mode also addresses a subsequently observed fault: `tunnel_only` left the original network's DNS servers configured, but they stopped answering through the tunnel. IP connectivity and an HTTPS request with an explicit destination IP still worked. Docker's kernel UDP networking then exposed a separate local DNS-port conflict; its fix is described below.
...
`pub on` verifies a complete unauthenticated 16 KiB upload to `api.anthropic.com` before reporting success: HTTP 401 is the expected application response, and curl must report the full request body as uploaded. A failed check triggers restoration of the captured WARP state. Existing `tunnel_only` leases remain restorable; an explicit `pub on` upgrades them to `warp+doh` over WireGuard while preserving their original restoration intent and twelve-hour ceiling.
...
`pub-preflight` leaves network settings unchanged and checks Docker's UDP setting when present, local port 53, WARP policy/state and DNS/HTTPS reachability. `pub-e2e` temporarily runs the repository's candidate controller directly from the checkout, checks a complete Anthropic upload with pub enabled, then verifies that `off` restores the original mode, tunnel protocol, protocol source, connection state and connectivity. It therefore tests the candidate before `make personal`; the installed controller is not used for the transition. The live test refuses an existing lease, takes the standard controller lock, requires the supervisor to be loaded, and keeps an isolated rollback snapshot for its guardian. A later run consumes any snapshot left by an interrupted or killed guardian before starting a new transition. Failures return a non-zero exit code. These commands are explicit rather than automatic deployment prerequisites because the live test switches the machine's network. They do not apply to a work machine without WARP.
...
Changing the WARP mode or tunnel protocol manually ends controller ownership and leaves the selected settings unchanged. A manual disconnect while both lease settings still match pauses the tunnel without discarding the restoration snapshot; `pub on` reconnects it and `pub off` still restores the original state.

`pub on` captures the pre-lease mode, tunnel protocol and source, and connection state as the restoration intent. A `Connecting` status is given ten seconds to settle first. `Connected` and `Disconnected` are restored exactly; a degraded state (`Unable to connect`, or still `Connecting` after that budget) is reported on activation and later restored best-effort — the controller aims to reconnect but ends the lease even if the network still refuses, the reconnection is rejected outright, or the resulting status cannot be read, rather than holding the pub tunnel open forever. Restoring the mode and protocol, and confirming both afterwards, stays strict for every intent. `Registration Missing` and any status the controller does not recognise are refused outright.
...
Interactive `pub on` and `pub off` operations wait up to ten seconds for another pub operation to release the controller lock, then report `another pub operation is already running` and exit 75 (`EX_TEMPFAIL`); the background reconciler does not wait. Pub gives changed WARP settings up to fifteen observations to become visible, activation up to 120 status probes, and interactive restoration up to sixty, printing progress on stderr only when it is a terminal — stdout carries the result alone, so scripted callers can match it. Background restoration also makes up to sixty probes in silence and retries on the next one-minute reconcile. Every `warp-cli` invocation has its own five-second timeout, so a stalled client cannot hold the controller lock indefinitely. The live-test guardian places an additional 120-second overall deadline around its independent fallback restoration. The controller rotates `~/Library/Logs/pub-lease.log` after it grows beyond 1 MiB, retaining one `.1` copy. Quarantined metadata is retained for at least seven days and removed by a later locked operation.
