...
from _agy_lib import env, io_json, proc

IMAGE_NAME: Final[str] = "agy-sandbox"
TARGET_DIR_NAME: Final[str] = ".devcontainer"

USAGE: Final[str] = (
    "Usage: agy_devcontainer.py <command> [options]\n"
    "\n"
    "Commands:\n"
    "  init [--minimal]   Copy devcontainer template to current project\n"
    "  build              Build the devcontainer image\n"
    "  run [options]      Run Antigravity CLI in the container\n"
    "\n"
    "Run options:\n"
    "  --bypass           Add --dangerously-skip-permissions to Antigravity CLI\n"
    "  --shell            Drop into zsh instead of launching Antigravity CLI\n"
)
...
def template_dir() -> Path:
    ...
    return Path(home) / ".gemini/antigravity-cli" / "templates" / "devcontainer"
...
def cmd_init(args: InitArgs, cwd: Path) -> CmdOutcome:
    ...
    msg = (
        ...
        + "\n"
        + "Next steps:\n"
        + "  agy-devcontainer build\n"
        + "  agy-devcontainer run\n"
    )
    ...
    if not (target / "Dockerfile").is_file():
        return CmdOutcome(
            ...
            stderr=(
                ...
                " Run 'agy_devcontainer.py init' first.\n"
            ),
        )
...
def _run_args(cwd: Path) -> list[str]:
    ...
    return [
        ...
        "-v",
        f"{settings}:/home/node/.gemini/antigravity-cli/settings.json:ro",
        "-v",
        "agy-shell-history:/home/node/.shell_history",
        "-w",
        "/home/node/workspace",
    ]
...
def cmd_run(args: RunArgs, cwd: Path) -> CmdOutcome:
    if not _image_exists(IMAGE_NAME):
        return CmdOutcome(
            exit_code=1,
            stderr=(
                f"ERROR: Image {IMAGE_NAME} not found. Run 'agy_devcontainer.py build' first.\n"
            ),
        )
