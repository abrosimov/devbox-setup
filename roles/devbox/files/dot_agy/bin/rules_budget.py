#!/usr/bin/env python3
"""Rules-budget instrumentation for the Antigravity CLI config tree (RI1).

Counts rule-like statements across ~/.gemini/antigravity-cli source (skills, agents, commands,
...
## Invocation
  ...
  uv run --project roles/devbox/files/dot_agy/bin \\
    python roles/devbox/files/dot_agy/bin/rules_budget.py [--json|--baseline PATH]
"""
...
# (research_pattern_language.md [45]: Anthropic AGENTS.md guidance).
BUDGET_LOW = 150
BUDGET_HIGH = 200
...
def _default_root() -> Path:
    ...
    # bin/ sits inside dot_agy/ (repo) or ~/.gemini/antigravity-cli/ (deployed).
    return here.parent
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Count rule-like statements across ~/.gemini/antigravity-cli config."
    )
    parser.add_argument(
        "--root",
        type=Path,
