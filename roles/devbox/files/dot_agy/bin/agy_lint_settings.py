#!/usr/bin/env python3
"""Diagnose mutually-exclusive rules in Antigravity CLI settings files.

Antigravity CLI merges permissions across three layers, low -> high precedence:

    ~/.gemini/antigravity-cli/settings.json            (user)
      -> <project>/.gemini/antigravity-cli/settings.json        (project)
        -> <project>/.gemini/antigravity-cli/settings.local.json  (local)
...
AGY_DIR: Final[str] = ".gemini/antigravity-cli"
RULE_RE: Final[re.Pattern[str]] = re.compile(r"^([A-Za-z_][A-Za-z0-9_]*)\((.*)\)$")

USAGE: Final[str] = (
    "Usage: agy_lint_settings.py [--all] [--json] [--quiet]\n"
    "  Report mutually-exclusive rules in Antigravity CLI settings files.\n"
    "\n"
    "Options:\n"
...
def deny_prefix(inner: str | None) -> str | None:
    ...
    Antigravity CLI only treats `:*` / ` *` as a wildcard at the *end* of a pattern;
...
def user_layer_path() -> Path:
    ...
    return Path(home) / AGY_DIR / "settings.json"


def find_project_agy_dir(start: Path) -> Path | None:
    current = start.resolve() if start.exists() else start
    if current.is_file():
        current = current.parent
    while True:
        candidate = current / AGY_DIR
        if candidate.is_dir():
            return candidate
...
def _project_layers(agy_dir: Path, *, include_user: bool) -> list[LoadedLayer]:
    layers: list[LoadedLayer] = []
    if include_user:
        user_path = user_layer_path()
        # Skip the user layer when the project *is* ~/.gemini/antigravity-cli to avoid double load.
        if user_path.parent.resolve() != agy_dir.resolve():
            layers.append(load_layer(user_path, "user"))
    layers.append(load_layer(agy_dir / "settings.json", "project"))
    layers.append(load_layer(agy_dir / "settings.local.json", "local"))
    return layers
...
def discover_agy_dirs(workspace: Path) -> list[Path]:
    return [
        candidate
        for entry in sorted(workspace.glob("*/"))
        for candidate in (entry / AGY_DIR, entry / "base" / AGY_DIR)
        if candidate.is_dir()
    ]


def _project_label(agy_dir: Path, workspace: Path) -> str:
    try:
        rel = agy_dir.relative_to(workspace)
        return str(rel.parent) if rel.parent != Path() else str(rel)
    except ValueError:
        return str(agy_dir.parent)
...
def run(argv: list[str], cwd: Path) -> tuple[int, list[ProjectReport] | str]:
    ...
    if parsed.scan_all:
        ...
        agy_dirs = discover_agy_dirs(workspace)
        if not agy_dirs:
            return 2, f"No projects with .gemini/antigravity-cli/ found under {workspace}"
        for agy_dir in agy_dirs:
            layers = _project_layers(agy_dir, include_user=True)
            reports.append(ProjectReport(_project_label(agy_dir, workspace), analyse(layers)))
    else:
        agy_dir = find_project_agy_dir(cwd)
        if agy_dir is None:
            return 2, "No .gemini/antigravity-cli/ directory found from the current path"
        layers = _project_layers(agy_dir, include_user=True)
        reports.append(ProjectReport(str(agy_dir.parent), analyse(layers)))
