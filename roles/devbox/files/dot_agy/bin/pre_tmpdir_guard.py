...
# Antigravity CLI's sandbox writeAllowOnly designates /tmp/agy and
# /private/tmp/agy as the canonical session-scratch namespace, and resolves
...
# settings.json (UV_CACHE_DIR=/tmp/agy/uv-cache, GOCACHE=/tmp/agy/..., ...).
_BARE_TMP_DIRS = frozenset({"/tmp", "/var/tmp"})  # noqa: S108


def _allowed_prefixes() -> tuple[str, ...]:
    prefixes = ["/tmp/agy/", "/private/tmp/agy/"]  # noqa: S108
...
def _extract_command_and_path(data: dict[str, object]) -> tuple[str, str]:
    ...
    # Antigravity CLI's hook wrapper sets ``$CC_BASH_COMMAND`` / ``$CC_TOOL_INPUT_FILE_PATH``.
...
def main() -> int:
    ...
    sys.stderr.write(
        ...
        "or the sandbox-allowed namespace (/tmp/agy/, /private/tmp/agy/, $TMPDIR).\n",
    )
    return hooks.BLOCK
