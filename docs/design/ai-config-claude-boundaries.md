...
- `scripts/ai_config/adapters.py:66` maps the Claude repository source `roles/devbox/files/dot_claude/settings.json`, user destination `.claude/settings.json`, and `settings.ai-config.json`. This is one source document and one destination, not a fragments loader.
...
- `scripts/ai_config/bindings.py:58` resolves profile, environment, Keychain and home bindings. The Claude manifest currently uses only the profile binding. No secret or home binding is declared there.
...
| `env`, 14 | Shared subtree; present | Manage named portable toolchain/cache/telemetry variables; leave unrelated application/private variables outside repository ownership. The current blanket rule admits every added descendant as shared. |
| `env.LANGFUSE_TRACING_ENVIRONMENT`, 18 | Environment; `profile:devbox_active_profile`; present | Retain generated profile binding. Live resolved value must not become the portable source declaration. |
| `extraKnownMarketplaces`, 23 | Shared subtree; present | Manage declared marketplace identities; distinguish optional user additions and CLI registry/cache state. Ansible marketplace registration and explicit user edits are distinct operations. |
| `model`, 27 | Shared; absent | Personal model choice or explicit portable default, depending on authority decision. Native `/model` is a documented writer; absence from source does not imply application state. |
| `permissions`, 31 | Shared subtree; present | Explicitly separate durable permission policy/defaults from user decisions. Currently all unlisted descendant keys inherit shared ownership; `defaultMode` is present. |
| `permissions.allow`, 35 | Shared ordered-set; present | Portable approved rules plus deliberately retained user additions, with an explicit revocation model. Saved standing approvals are normally project-local in current documentation, not evidence that every user allow entry is application-generated. |
| `permissions.deny`, 40 | Shared ordered-set; present | Durable restrictions; define who may remove a rule. Native cross-scope deny semantics differ from this reconciler's set reconciliation. |
| `permissions.additionalDirectories`, 45 | Shared ordered-set; absent | Machine/workspace grants are candidates for local ownership or named bindings. Do not classify all future entries as portable simply because this rule exists. |
| `sandbox`, 50 | Shared subtree; present | Separate selected sandbox choices from host paths and network exceptions. Domain/excluded-command arrays are presently atomic in reconciliation, unlike permission arrays. |
| `skipAutoPermissionPrompt`, 54 | Shared; present | Unknown product contract from the consulted current primary pages. Do not rename, remove or reclassify based only on its name. |
| `skipWorkflowUsageWarning`, 58 | Runtime; absent | Existing runtime exception. Exact product semantics and writer remain unverified; preserve under current contract until established. |
| `statusLine`, 62 | Shared subtree; present | Portable helper definition with machine installation binding where necessary; explicit manual edits are another possible writer. |
| `tui`, 66 | Shared; present, scalar `fullscreen` | Personal UI preference or portable default. The source is scalar, so there is no current subtree granularity to split. Exact installed persistence behaviour was not exercised. |

There are **15 manifest rules** and **9 top-level keys in the current settings source**. The next inventory accounts for every source key independently of manifest declarations.

| Repository top-level key | Source location | Current material / split implication |
| --- | --- | --- |
| `env` | `settings.json:2` | Toolchain choices, cache paths, OTLP configuration, local Langfuse bridge parameters and one profile declaration. Values are repository-recorded; this does not establish that future similarly named values are non-secret. |
| `permissions` | `settings.json:33` | `allow`, `deny`, `defaultMode`. `additionalDirectories` is not present. |
| `statusLine` | `settings.json:399` | Command helper at `~/.claude/bin/statusline.py`. Installation of the helper and configuration of its path must remain coherent. |
| `enabledPlugins` | `settings.json:403` | Five entries: three official LSP plugins and two Anthropic skills plugins. Native installation/enabling also occurs outside ai-config. |
| `extraKnownMarketplaces` | `settings.json:410` | `anthropic-agent-skills` points to the `anthropics/skills` GitHub source. |
| `sandbox` | `settings.json:418` | Enablement, automatic sandbox permission choice, unsandboxed allowance, network domains/local binding, weaker network isolation and excluded commands. |
| `autoMemoryEnabled` | `settings.json:535` | Explicit false. |
| `skipAutoPermissionPrompt` | `settings.json:536` | Explicit true; consulted documentation did not establish its contract. |
| `tui` | `settings.json:537` | Explicit `fullscreen`. |
...
Use isolated fixtures and a version-recorded native client to check scalar precedence; nested-key preservation; permission addition and revocation; plugin true/false and uninstall distinction; unknown UI-key preservation; profile/secret binding non-capture; hook source visibility and duplicate execution; hot reload versus new-session changes; and launch coverage outside the shell wrapper. Keep existing tests for transaction rollback and concurrent source changes. These are proposed future checks, not completed validation.

## Repository source navigation

Return to the [ownership and composition study](ai-config-ownership-and-composition.md).

- [Claude manifest](../../roles/devbox/files/dot_claude/settings.ai-config.json), [source](../../roles/devbox/files/dot_claude/settings.json) and [assets](../../roles/devbox/files/dot_claude/).
