...
1. Full-rewrite-on-feedback: when the user iterates on a numbered proposal,
   Antigravity emits a four-screen rewrite instead of a delta. The user cannot
   tell what changed.
...
decide whether to inject ``additionalContext`` reminding Antigravity to operate
in delta-only / preserve-numbering mode. False positives degrade to a mild
nudge Antigravity can ignore; false negatives leave the existing behaviour
...
def dispatch(payload: Mapping[str, object]) -> str | None:
    ...
    if event == "Stop":
        # ``stop_hook_active`` is set by Antigravity CLI when a Stop hook already
